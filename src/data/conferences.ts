export type ConferenceCategory = 'Core HCI' | 'XR / 3D Interaction' | 'Design / UbiComp';
export type ConferenceStatus =
  | 'Submission'
  | 'Abstract'
  | 'Paper'
  | 'Review'
  | 'Rebuttal'
  | 'Notification'
  | 'Camera'
  | 'Presentation-Video'
  | 'Registration'
  | 'In session'
  | 'TBA';

export type MilestoneKind = 'Abstract' | 'Paper' | 'Notification' | 'Conference';
export type ConferenceRank = 'A*' | 'A' | 'B' | 'Journal published' | 'Not ranked';

export interface ConferenceMilestone {
  kind: MilestoneKind;
  date: string;
  endDate?: string;
  label: string;
  cutoffTime?: string;
  timeZone?: 'AoE';
}

export interface ConferenceVenue {
  id: string;
  name: string;
  fullName: string;
  category: ConferenceCategory;
  status: ConferenceStatus;
  location: string;
  sourceUrl: string;
  verified: boolean;
  rank: ConferenceRank;
  rankUrl: string;
  logo: string;
  logoAlt: string;
  tags: string[];
  milestones: ConferenceMilestone[];
}

export const conferenceVenues: ConferenceVenue[] = [
  {
    id: 'chi-2027',
    name: 'CHI 2027',
    fullName: 'ACM Conference on Human Factors in Computing Systems',
    category: 'Core HCI',
    status: 'Paper',
    location: 'Pittsburgh, USA',
    sourceUrl: 'https://chi2027.acm.org/authors/papers/',
    verified: true,
    rank: 'A*',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1053/',
    logo: '/assets/conferences/chi-2027.png',
    logoAlt: 'ACM CHI logo',
    tags: ['HCI', 'Interaction Design', 'User Interfaces'],
    milestones: [
      {
        kind: 'Abstract',
        date: '2026-08-13',
        label: 'Submission site opens',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      {
        kind: 'Paper',
        date: '2026-09-10',
        label: 'Full paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2026-11-05', label: 'Reviews released' },
      {
        kind: 'Paper',
        date: '2026-12-03',
        label: 'Revised paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2026-12-17', label: 'Final notification' },
      {
        kind: 'Paper',
        date: '2027-02-11',
        label: 'Camera-ready paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2027-05-10', endDate: '2027-05-14', label: 'Conference' },
    ],
  },
  {
    id: 'iui-2027',
    name: 'IUI 2027',
    fullName: 'ACM Conference on Intelligent User Interfaces',
    category: 'Core HCI',
    status: 'Review',
    location: 'Helsinki, Finland',
    sourceUrl: 'https://iui.acm.org/2027/',
    verified: true,
    rank: 'A',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1101/',
    logo: '/assets/conferences/iui-2027.png',
    logoAlt: 'IUI 2027 logo',
    tags: ['Human-AI Interaction', 'Intelligent Interfaces'],
    milestones: [
      {
        kind: 'Abstract',
        date: '2026-08-13',
        label: 'Abstract submission',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      {
        kind: 'Paper',
        date: '2026-08-20',
        label: 'Full paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2026-10-29', label: 'Initial notification' },
      { kind: 'Notification', date: '2026-11-23', label: 'Final notification' },
      { kind: 'Conference', date: '2027-03-15', endDate: '2027-03-18', label: 'Conference' },
    ],
  },
  {
    id: 'ieee-vr-2027',
    name: 'IEEE VR 2027',
    fullName: 'IEEE Conference on Virtual Reality and 3D User Interfaces',
    category: 'XR / 3D Interaction',
    status: 'Review',
    location: 'Sydney, Australia',
    sourceUrl: 'https://ieeevr.org/2027/contribute/papers/',
    verified: true,
    rank: 'A*',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/?by=all&search=IEEE+VR&source=CORE2026',
    logo: '/assets/conferences/ieee-vr-2027.webp',
    logoAlt: 'IEEE VR 2027 logo',
    tags: ['VR', 'AR / MR', '3DUI', 'Spatial Computing'],
    milestones: [
      {
        kind: 'Abstract',
        date: '2026-08-24',
        label: 'Required abstract deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      {
        kind: 'Paper',
        date: '2026-08-31',
        label: 'Full paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2026-11-30', label: 'Initial notification' },
      { kind: 'Notification', date: '2027-01-07', label: 'Final notification' },
      {
        kind: 'Paper',
        date: '2027-01-21',
        label: 'Camera-ready paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2027-03-20', endDate: '2027-03-24', label: 'Conference' },
    ],
  },
  {
    id: 'sui-2026',
    name: 'SUI 2026',
    fullName: 'ACM Symposium on Spatial User Interaction',
    category: 'XR / 3D Interaction',
    status: 'Registration',
    location: 'Bari, Italy',
    sourceUrl: 'https://sui.acm.org/2026/',
    verified: true,
    rank: 'Not ranked',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/',
    logo: '/assets/conferences/sui-2026.png',
    logoAlt: 'ACM SUI 2026 logo',
    tags: ['Spatial Interaction', 'XR', '3D UI'],
    milestones: [
      {
        kind: 'Paper',
        date: '2026-08-18',
        label: 'Camera-ready paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2026-10-10', endDate: '2026-10-11', label: 'Conference' },
    ],
  },
  {
    id: 'ismar-2026',
    name: 'ISMAR 2026',
    fullName: 'IEEE International Symposium on Mixed and Augmented Reality',
    category: 'XR / 3D Interaction',
    status: 'Presentation-Video',
    location: 'Bari, Italy',
    sourceUrl: 'https://www.ieeeismar.net/2026/',
    verified: true,
    rank: 'A*',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/?by=all&search=ISMAR&source=CORE2026',
    logo: '/assets/conferences/ismar-2026.png',
    logoAlt: 'ISMAR 2026 logo',
    tags: ['AR / MR', 'XR', 'Computer Vision'],
    milestones: [
      {
        kind: 'Paper',
        date: '2026-09-04',
        label: 'Presentation video & camera deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2026-10-05', endDate: '2026-10-09', label: 'Conference' },
    ],
  },
  {
    id: 'uist-2026',
    name: 'UIST 2026',
    fullName: 'ACM Symposium on User Interface Software and Technology',
    category: 'Core HCI',
    status: 'Registration',
    location: 'Detroit, USA',
    sourceUrl: 'https://uist.acm.org/uist2026/',
    verified: true,
    rank: 'A*',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/66/',
    logo: '/assets/conferences/uist.png',
    logoAlt: 'ACM UIST 2026 logo',
    tags: ['Interactive Systems', 'Interface Technology', 'Hardware'],
    milestones: [
      {
        kind: 'Paper',
        date: '2026-08-28',
        label: 'Camera-ready paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      {
        kind: 'Paper',
        date: '2026-09-04',
        label: 'Registration deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2026-11-02', endDate: '2026-11-05', label: 'Conference' },
    ],
  },
  {
    id: 'tei-2027',
    name: 'TEI 2027',
    fullName: 'ACM Conference on Tangible, Embedded and Embodied Interaction',
    category: 'Design / UbiComp',
    status: 'Review',
    location: 'Lisbon, Portugal',
    sourceUrl: 'https://tei.acm.org/2027/call-for-papers.html',
    verified: true,
    rank: 'B',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1798/',
    logo: '/assets/conferences/tei-2027.png',
    logoAlt: 'TEI 2027 Lisbon logo',
    tags: ['Tangible Interaction', 'Embodied Interaction'],
    milestones: [
      {
        kind: 'Paper',
        date: '2026-08-06',
        label: 'Paper and pictorial deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2026-10-22', label: 'Author notification' },
      { kind: 'Conference', date: '2027-03-01', endDate: '2027-03-04', label: 'Conference' },
    ],
  },
  {
    id: 'iss-2026',
    name: 'ISS 2026',
    fullName: 'ACM International Conference on Interactive Surfaces and Spaces',
    category: 'Design / UbiComp',
    status: 'Review',
    location: 'Turin, Italy',
    sourceUrl: 'https://iss.acm.org/2026/',
    verified: true,
    rank: 'A',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1100/',
    logo: '/assets/conferences/iss-2026.png',
    logoAlt: 'ACM ISS 2026 logo',
    tags: ['Interactive Surfaces', 'Spatial Interfaces'],
    milestones: [
      {
        kind: 'Paper',
        date: '2026-07-22',
        label: 'Summer round paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2026-09-15', label: 'Second phase notification' },
      { kind: 'Conference', date: '2026-11-23', endDate: '2026-11-26', label: 'Conference' },
    ],
  },
  {
    id: 'vrst-2026',
    name: 'VRST 2026',
    fullName: 'ACM Symposium on Virtual Reality Software and Technology',
    category: 'XR / 3D Interaction',
    status: 'Camera',
    location: 'Sendai, Japan',
    sourceUrl: 'https://vrst.acm.org/vrst2026/cfp/',
    verified: true,
    rank: 'B',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/67/',
    logo: '/assets/conferences/vrst-2026.png',
    logoAlt: 'VRST 2026 logo',
    tags: ['VR / AR', 'XR Systems', 'Real-Time Graphics'],
    milestones: [
      {
        kind: 'Abstract',
        date: '2026-06-17',
        label: 'Paper abstract deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      {
        kind: 'Paper',
        date: '2026-06-24',
        label: 'Full paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2026-08-28', label: 'Author notification' },
      {
        kind: 'Paper',
        date: '2026-09-18',
        label: 'Camera-ready paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2026-11-16', endDate: '2026-11-18', label: 'Conference' },
    ],
  },
  {
    id: 'icmi-2026',
    name: 'ICMI 2026',
    fullName: 'ACM International Conference on Multimodal Interaction',
    category: 'XR / 3D Interaction',
    status: 'Registration',
    location: 'Naples, Italy',
    sourceUrl: 'https://icmi.acm.org/2026/',
    verified: true,
    rank: 'A',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1098/',
    logo: '/assets/conferences/icmi-2026.png',
    logoAlt: 'ACM ICMI 2026 logo',
    tags: ['Multimodal Interaction', 'Affective Computing', 'Gaze & Gesture'],
    milestones: [
      {
        kind: 'Paper',
        date: '2026-04-23',
        label: 'Full paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2026-07-09', label: 'Author notification' },
      {
        kind: 'Paper',
        date: '2026-08-10',
        label: 'Camera-ready paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2026-10-05', endDate: '2026-10-09', label: 'Conference' },
    ],
  },
  {
    id: 'etra-2027',
    name: 'ETRA 2027',
    fullName: 'ACM Symposium on Eye Tracking Research & Applications',
    category: 'XR / 3D Interaction',
    status: 'Submission',
    location: 'Pamplona, Spain',
    sourceUrl: 'https://etra.acm.org/2027/',
    verified: true,
    rank: 'B',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1042/',
    logo: '/assets/conferences/etra-2027.png',
    logoAlt: 'ACM ETRA 2027 logo',
    tags: ['Eye Tracking', 'Gaze Interaction', 'XR'],
    milestones: [
      {
        kind: 'Paper',
        date: '2027-01-14',
        label: 'Short & full paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2027-03-11', label: 'Author notification' },
      { kind: 'Conference', date: '2027-06-07', endDate: '2027-06-10', label: 'Conference' },
    ],
  },
  {
    id: 'gi-2027',
    name: 'GI 2027',
    fullName: 'Graphics Interface Conference 2027',
    category: 'Core HCI',
    status: 'Submission',
    location: 'Winnipeg, Canada',
    sourceUrl: 'https://graphicsinterface.org/conference/2027/',
    verified: true,
    rank: 'B',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1040/',
    logo: '/assets/conferences/gi-2027.png',
    logoAlt: 'Graphics Interface 2027 logo',
    tags: ['Computer Graphics', 'Human-Computer Interaction'],
    milestones: [
      {
        kind: 'Paper',
        date: '2026-12-15',
        label: 'Full paper submission deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2027-06-15', endDate: '2027-06-18', label: 'Conference' },
    ],
  },
  {
    id: 'dis-2027',
    name: 'DIS 2027',
    fullName: 'ACM Designing Interactive Systems Conference',
    category: 'Design / UbiComp',
    status: 'Submission',
    location: 'Stockholm, Sweden',
    sourceUrl: 'https://dis.acm.org/2027/',
    verified: true,
    rank: 'A',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/422/',
    logo: '/assets/conferences/dis-2027.png',
    logoAlt: 'DIS 2027 logo',
    tags: ['Interaction Design', 'Research through Design'],
    milestones: [
      {
        kind: 'Paper',
        date: '2027-01-28',
        label: 'Full paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2027-04-15', label: 'Author notification' },
      { kind: 'Conference', date: '2027-06-28', endDate: '2027-07-02', label: 'Conference' },
    ],
  },
  {
    id: 'cscw-2027',
    name: 'CSCW 2027',
    fullName: 'ACM Conference on Computer-Supported Cooperative Work',
    category: 'Core HCI',
    status: 'Submission',
    location: 'Seattle, USA',
    sourceUrl: 'https://cscw.acm.org/',
    verified: true,
    rank: 'A',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/13/',
    logo: '/assets/conferences/cscw.webp',
    logoAlt: 'ACM CSCW logo',
    tags: ['Collaboration', 'Social Computing'],
    milestones: [
      {
        kind: 'Paper',
        date: '2026-10-14',
        label: 'PACMHCI October Cycle submission',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      {
        kind: 'Paper',
        date: '2027-01-13',
        label: 'PACMHCI January Cycle submission',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2027-11-06', endDate: '2027-11-10', label: 'Conference' },
    ],
  },
  {
    id: 'mobilehci-2027',
    name: 'MobileHCI 2027',
    fullName: 'ACM International Conference on Mobile Human-Computer Interaction',
    category: 'Design / UbiComp',
    status: 'Submission',
    location: 'Taipei, Taiwan',
    sourceUrl: 'https://mobilehci.acm.org/',
    verified: true,
    rank: 'B',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1054/',
    logo: '/assets/conferences/mobilehci-2026.png',
    logoAlt: 'MobileHCI logo',
    tags: ['Mobile HCI', 'Ubiquitous Computing'],
    milestones: [
      {
        kind: 'Paper',
        date: '2027-02-18',
        label: 'Full paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2027-05-20', label: 'Author notification' },
      { kind: 'Conference', date: '2027-09-27', endDate: '2027-09-30', label: 'Conference' },
    ],
  },
  {
    id: 'ubicomp-2027',
    name: 'UbiComp / ISWC 2027',
    fullName: 'ACM International Joint Conference on Pervasive and Ubiquitous Computing',
    category: 'Design / UbiComp',
    status: 'Submission',
    location: 'Cancún, Mexico',
    sourceUrl: 'https://www.ubicomp.org/',
    verified: true,
    rank: 'Journal published',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1825/',
    logo: '/assets/conferences/ubicomp-2026.png',
    logoAlt: 'UbiComp ISWC logo',
    tags: ['Ubiquitous Computing', 'Wearables', 'Sensors'],
    milestones: [
      {
        kind: 'Paper',
        date: '2027-02-15',
        label: 'IMWUT February Cycle submission',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      {
        kind: 'Paper',
        date: '2027-05-15',
        label: 'IMWUT May Cycle submission',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2027-10-10', endDate: '2027-10-14', label: 'Conference' },
    ],
  },
  {
    id: 'autoui-2027',
    name: 'AutomotiveUI 2027',
    fullName: 'ACM International Conference on Automotive User Interfaces',
    category: 'Design / UbiComp',
    status: 'Submission',
    location: 'Munich, Germany',
    sourceUrl: 'https://auto-ui.acm.org/2027/',
    verified: true,
    rank: 'B',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1802/',
    logo: '/assets/conferences/autoui-2027.png',
    logoAlt: 'AutomotiveUI 2027 logo',
    tags: ['Automotive UI', 'Vehicular HCI', 'Autonomous Driving'],
    milestones: [
      {
        kind: 'Paper',
        date: '2027-04-08',
        label: 'Full paper submission deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2027-06-17', label: 'Author notification' },
      { kind: 'Conference', date: '2027-09-19', endDate: '2027-09-22', label: 'Conference' },
    ],
  },
  {
    id: 'siggraph-asia-2026',
    name: 'SIGGRAPH Asia 2026',
    fullName: 'ACM SIGGRAPH Conference and Exhibition on Computer Graphics in Asia',
    category: 'XR / 3D Interaction',
    status: 'Camera',
    location: 'Daejeon, South Korea',
    sourceUrl: 'https://sa2026.siggraph.org/',
    verified: true,
    rank: 'A*',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1039/',
    logo: '/assets/conferences/siggraph-asia-2026.png',
    logoAlt: 'SIGGRAPH Asia 2026 logo',
    tags: ['Computer Graphics', 'XR', '3D Interaction', 'Animation'],
    milestones: [
      {
        kind: 'Paper',
        date: '2026-09-15',
        label: 'Camera-ready paper deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Conference', date: '2026-11-24', endDate: '2026-11-27', label: 'Conference' },
    ],
  },
  {
    id: 'hri-2027',
    name: 'HRI 2027',
    fullName: 'ACM/IEEE International Conference on Human-Robot Interaction',
    category: 'Core HCI',
    status: 'Submission',
    location: 'Seattle, USA',
    sourceUrl: 'https://humanrobotinteraction.org/2027/',
    verified: true,
    rank: 'A*',
    rankUrl: 'https://portal.core.edu.au/conf-ranks/1097/',
    logo: '/assets/conferences/hri-2027.png',
    logoAlt: 'HRI 2027 logo',
    tags: ['Human-Robot Interaction', 'Social Robotics', 'HCI'],
    milestones: [
      {
        kind: 'Paper',
        date: '2026-10-01',
        label: 'Full paper submission deadline',
        cutoffTime: '23:59',
        timeZone: 'AoE',
      },
      { kind: 'Notification', date: '2026-12-08', label: 'Author notification' },
      { kind: 'Conference', date: '2027-03-08', endDate: '2027-03-11', label: 'Conference' },
    ],
  },
];
